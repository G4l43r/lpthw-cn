...
file that you will run, but the *other* is named ``ex15_sample.txt``.  This second
...
and parameters.  Just like with ``open`` and ``raw_input``.  The difference
is that when you say ``txt.read()`` you are saying, "Hey txt! Do your read command
with no parameters!"
