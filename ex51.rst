...
write a small little test that tests your web application form based on what
you learned in Exercise 47.  If you don't remember Exercise 47 then go back
and read it again.
...
``bin/app.py`` module, and then running it manually.  The ``lpthw.web`` framework
...
Which means you can pass in the URL as the first parameter, then you can change the 
method of the request, and what form data you send, even the host and the headers.
...
To validate responses from this function, you use the ``assert_response`` function
...
You just pass in the response you get from calling ``app.request`` and then 
add things you want checked.  You can use the ``contains`` parameter to make 
sure that the response contains certain values.  You can use the ``status``
parameter to check for certain responses.  There's actually quite a lot of
information in this small little function so it would be good for you to
study it.
...
Your next task should be to spend some time studying this latest application
and especially how the automated testing works.  Make sure you understand how I 
imported the application from ``bin/app.py`` and ran it directly for the automated
test.  This is an important trick you should understand and will lead to 
more learning.
