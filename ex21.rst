...
You've been using the ``=`` character to name variables and set them
...
3. Then we tell Python to do something kind of backwards, we return
   the addition of ``a + b``.  You might say this as, "I add ``a`` and ``b``
   then return them."
4. Python adds the two numbers, and then when the function ends any line
...
break it down and try trace what's going on.  To help there's extra
...
2. At the end of the script is a puzzle.  I'm taking the return value
   ...
   the functios.  It looks really weird, but if you ran the script you 
...
4. Finally, do the inverse, write out a simple formula and use the 
   functions in the same way to calculate it.  First do it with variables
   then with functions called as arguments to functions.

This exercise might really whack your brain out, but take it slow and easy
and treat it like a little game.  Figuring things like this out is 
almost all of the fun there is in programming, so I'll be giving you
more little problems like this as we go.
