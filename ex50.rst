...
is install the "web framework" called ``web.py``.  The term "framework" generally
...
In our case, we'll be using the ``web.py`` framework, but there are many, many,
*many* others you can choose.  For now, learn ``web.py`` then branch out to another
one when you're ready (or just keep using ``web.py`` since it's good enough).

Using ``pip`` install ``web.py``:
...
project directory using ``web.py``.  First you'll need to make your project directory:
...
``web.py`` application possible.  Put the following code into ``bin/app.py``:
...
Those are log messages that ``web.py`` prints out so you can see that the server is working, and
...
I'm going to have you break your web.py application in various ways and then restructure it 
...
3. Inside ``bin/app.py`` you've got a list of URLs and what classes they match.  The only one
   ...
   with a browser, ``web.py`` will find the ``class index`` and load it to handle the
   request.
4. Now that ``web.py`` has found ``class index`` it calls the ``index.GET`` method on an
   instance of that class to actually handle the request.  This function runs, and simply
   returns a string for what ``web.py`` should send to the browser.
5. Finally, ``web.py`` has handled the request and sends this response to the browser which
   is what you are seeing.

Make sure you really understand this, and you should draw up a diagram of how this information
is flowing from your browser, to ``web.py``, then to ``index.GET`` and back to your browser.
...
``web.py`` gives you this nice error page to track down exactly where.  Try doing each
...
3. Try breaking this simple application in other ways and explore what happens.  Don't forget to also
   look at the logs being printed into your terminal as ``web.py`` will put other stack traces
...
You can break your web.py application, but did you notice that "Hello World" isn't a very
...
however is a *template*, which means that ``web.py`` will fill in "holes" in the text
...
To make your ``bin/app.py`` do this, you need to add some code to tell web.py where to
...
You should spend a good week really diving into the http://webpy.org/ documentation and trying
everything you can find there to try.  Rework this initial project directory to add anything
you can think up and explore.  You should also be learning about HTML5 and CSS3 and trying
to get a good understanding of how to write those kinds of files.  We'll be making a very
pathetic set of HTML5 and CSS3 files in the last two exercises, but for now you should be
